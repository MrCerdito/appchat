...
exports.encryptedTextTransformer = void 0;
...
const logger = new common_1.Logger('Encryption');
let keyWarningLogged = false;
function logKeyWarning() {
    if (!keyWarningLogged) {
        logger.warn('CHAT_ENCRYPTION_KEY no está configurada. ' +
...
function deriveKeyV1(raw) {
    return (0, crypto_1.createHash)('sha256').update(raw).digest();
}
function deriveKeyV2(raw, salt) {
    return (0, crypto_1.pbkdf2Sync)(raw, salt, PBKDF2_ITERATIONS, PBKDF2_KEYLEN, PBKDF2_DIGEST);
}
function getKey() {
...
exports.encryptedTextTransformer = {
    to(value) {
        if (value == null)
            return null;
...
//# sourceMappingURL=encrypted-text.transformer.js.map