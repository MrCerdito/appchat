...
export class SessionsService {
  ...
  async getMetricsByAdvisor(advisorId: string) {
    ...
    const totalCerradas = await this.sessionRepo.count({
      where: { advisor: { id: advisorId }, status: 'closed' },
    });

    const totalActivas = await this.sessionRepo.count({
      where: { advisor: { id: advisorId }, status: 'active' },
    });

    const hoyAtendidas = await this.sessionRepo
      .createQueryBuilder('s')
      .where('s.advisor_id = :id',      { id: advisorId })
      .andWhere('s.created_at >= :hoy', { hoy })
      .getCount();

    const semanaAtendidas = await this.sessionRepo
      .createQueryBuilder('s')
      .where('s.advisor_id = :id',         { id: advisorId })
      .andWhere('s.created_at >= :inicio', { inicio: inicioSemana })
      .getCount();

    const cerradas = await this.sessionRepo.find({
      where:  { advisor: { id: advisorId }, status: 'closed' },
    ...
    for (const s of sessions) {
      const primerMensaje = await this.messageRepo.findOne({
        where: { session: { id: s.id }, senderType: 'advisor' },
        order: { createdAt: 'ASC' },
      });
      if (primerMensaje) {
        totalPrimeraRespuesta += new Date(primerMensaje.createdAt).getTime() - new Date(s.createdAt).getTime();
  ...
  async getRankingAsesores() {
    ...
    const ranking = await Promise.all(
      advisors.map(async (advisor) => {
        const total = await this.sessionRepo.count({
          where: { advisor: { id: advisor.id } },
        });
        const totalCerradas = await this.sessionRepo.count({
          where: { advisor: { id: advisor.id }, status: 'closed' },
        });
        const ratings = await this.ratingRepo
          .createQueryBuilder('r')
          .innerJoin('r.session', 's')
          .where('s.advisor_id = :id', { id: advisor.id })
          .getMany();

        const totalRatings = ratings.length;
        const avgEstrellas = totalRatings > 0
          ? Math.round((ratings.reduce((acc, r) => acc + r.estrellas, 0) / totalRatings) * 10) / 10
          : 0;

        return {
          id: advisor.id, name: advisor.name, status: advisor.status,
          activeChats: advisor.activeChats, total, totalCerradas, totalRatings, avgEstrellas,
        };
      })
    );
