...
get rolLabel(): string {
  ...
  private iniciarTimerInactividadIa(): void {
    ...
    console.log('[Timer] Iniciado. Aviso en', AVISO_MS / 1000, 's. Cierre en', TOTAL_MS / 1000, 's');

    // Paso 1: mostrar overlay con barra regresiva.
    this.inactividadIaAviso$ = setTimeout(() => {
      if (!this.aiMode || this.step !== 'chat') {
        console.log('[Timer] Aviso cancelado — fuera de contexto');
        return;
      }
      console.log('[Timer] ✅ Overlay visible');
    ...
    this.inactividadIaTimer = setTimeout(() => {
      if (!this.aiMode || this.step !== 'chat') {
        console.log('[Timer] Cierre cancelado — fuera de contexto');
        return;
      }

      console.log('[Timer] ✅ Cerrando sesión por inactividad');
      this._limpiarTimersInactividad();
      ...
      if (this.session?.id) {
        this.sessionService.close(this.session.id).subscribe({
          next : () => console.log('[Timer] Backend: sesión cerrada OK'),
      ...
      setTimeout(() => {
        console.log('[Timer] Redirigiendo al formulario...');
        this.clearSession();
      }, 2000);
