...
reactionSummaryLabel(msg: WaMessage, messages: WaMessage[]): string {
  ...
  async triggerAI(): Promise<void> {
    ...
    console.log('Mensaje a mejorar:', draft);
    ...
    console.log('Estado: enviando solicitud a la IA...');
    ...
    try {
      ...
      console.log('Estado: IA respondio correctamente.');
      console.log('Texto mejorado:', improved);
      console.log(`Tiempo: ${Math.round(performance.now() - startedAt)} ms`);
      ...
      console.log(`Tiempo hasta error: ${Math.round(performance.now() - startedAt)} ms`);
      this.sendError = 'No se pudo mejorar el texto con IA. Intenta de nuevo.';
    } finally {
      this.isAiGenerating = false;
      console.log('Estado: finalizado, boton desbloqueado.');
      console.groupEnd();
      this.messageInput?.nativeElement?.focus();
    }
  }
