...
export class SocketService {
  ...
  console.log('[Socket] connect() llamado, token:', token ? 'con token' : 'sin token', '| socket existe:', !!this.socket, '| currentToken igual:', this.currentToken === token);
  ...
  console.log(`[Socket] on('${event}') — subject existe: ${this.listeners.has(event)} | socket existe: ${!!this.socket}`);
  ...
  if (!this.listeners.has(event)) {
    ...
    if (this.socket) {
      console.log(`[Socket] registrando listener físico para: ${event}`);
      this.socket.on(event, (data: T) => {
        console.log(`[Socket] evento recibido: ${event}`, data);
        subject.next(data);
      });
    } else {
      console.log(`[Socket] SIN SOCKET — listener para ${event} se registrará al conectar`);
    }
  } else if (this.socket && !this.socket.hasListeners(event)) {
    console.log(`[Socket] re-registrando listener físico para: ${event}`);
