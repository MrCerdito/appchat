...
export class WhatsappChatService implements OnDestroy {
  ...
  loadChats(page = 1, limit = 50): Observable<WaChat[] | { chats: WaChat[]; total: number; hasMore: boolean }> {
    return this.http.get<WaChat[] | { chats: WaChat[]; total: number; hasMore: boolean }>(
      ...
      tap(res => {
        ...
        if (page === 1) this.refreshUnreadTotal().subscribe();
      }),
      catchError(() => {
        return of([] as WaChat[]);
      }),
  ...
  refreshUnreadTotal(): Observable<number> {
    return this.http.get<{ total: number }>(
      `${this.apiUrl}/chats/unread-total`,
      { headers: this.headers() },
    ).pipe(
      map(res => Number(res?.total ?? 0)),
      tap(total => this.unreadTotal$.next(total)),
      catchError(() => of(this.unreadTotal$.getValue())),
    );
  }
